...
Detector::Detector(string xml_path, double cof_threshold,
                   double nms_area_threshold, string deviceName,
                   vector<string> labels) :
 _xml_path(xml_path), _cof_threshold(cof_threshold),
 _nms_area_threshold(nms_area_threshold), _deviceName(deviceName),
 _labels(labels) {}
...
bool Detector::init(){
    ...
    //获取可执行网络
    _network =  ie.LoadNetwork(cnnNetwork, _deviceName);
    return true;
}
...
//处理图像获取结果
bool Detector::process_frame(Mat& img,vector<Object>& detected_objects){
    if(img.empty()){
        cout << "无效图片输入" << endl;
        return false;
    }
    cv::Mat inframe = img.clone();
